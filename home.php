<?php 
	...
	if (isset($_POST['tweet']))
	{
		...
		if (!empty($status) or !empty($_FILES['file']['name'][0]))
		{
			...
			$tweetID = $getFromU->create('tweets', array('status' => $status, 'tweetBy' => $user_id, 'tweetImage' => $tweetImage, 'postedOn' => date('Y-m-d H:i:s')));
			preg_match_all("/#+([a-zA-Z0-9_]+)/i",$status, $hashtag);
			if(!empty($hashtag))
			{
				$getFromT->addTrend($status);
			}
			$getFromT->addMention($status,$user_id,$tweetID);
		}
		else
		{
	...
	<div class="nav">
		...
		<div class="nav-left">
			<ul>
				...
				<li><a href="i/notifications"><i class="fa fa-bell" aria-hidden="true"></i>Notifications<span id="notification"><?php if($notification->totalN > 0){echo '<span class="span-i">'.$notification->totalN.'</span>';} ?></span></a></li>
 	...
 	<!--==TRENDS==-->
		...
		<div class="in-center">
			<div class="in-center-wrap">
				...
				<button class="deleteComment" data-tweet="'.$tweet->tweetID.'" data-comment="'.$comment->commentID.'"></button>
				<!--Tweet SHOW WRAPPER-->
				 <div class="tweets">
