...
class Message extends User
{
            ...
            else
            {
                ...
                <div class="main-msg-body-left">
                    <div class="main-msg-l">
                        <div class="msg-img-l">
                            <a href="#"><img src="'.$message->profileImage.'"/></a>
                        </div>
    ...
    public function deleteMsg($messageID, $user_id)
    {
        ...
        // $stmt -> bindPram(":messageID",$messageID,PDO::PARAM_INT);
        // $stmt -> bindPram(":user_id",$user_id,PDO::PARAM_INT);
        // $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare("SELECT COUNT(messageID) AS totalM, (SELECT COUNT(notificationID) FROM notification WHERE notificationFor = :user_id AND status = '0') AS totalN FROM messages WHERE messageTo = :user_id AND status = '0'");
        $stmt->bindParam(":user_id",$user_id,PDO::PARAM_INT);
        $stmt->execute();
    ...
    public function sendNotification($getID, $user_id, $target, $type)
    {
        $date = date('Y-m-d H:i:s');
        $this->create('notification', array('notificationFor' => $getID, 'notificationFrom' => $user_id, 'target' => $target, 'type' => $type, 'time' => $date));
    }
}
