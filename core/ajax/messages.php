<?php 
    ...
    {
        $user_id = $_SESSION['user_id'];
        $messageForm = $_POST['showChatMessage'];
        $messages = $getFromM->getMessages($messageForm,$user_id);
    }
    if(isset($_POST['showMessage']) && !empty($_POST['showMessage']))
    {
        $user_id = $_SESSION['user_id'];
        $messages = $getFromM->recentMessages($user_id);
        $getFromM->messageView($user_id);
        ?>
                    ...
                    <div class="back-inner">
                        ...
                        <!--Direct Messages-->
                            <div class="people-message" data-user="<?php echo $message->user_id ?>">
                                <div class="people-inner">
                                    <div class="people-img">
                                        <img src="<?php echo BASE_URL.$messages->profileImage ?>"/>
                                    </div>
                                    <div class="name-right2">
                                        <span><a href="#"><?php echo $messages->screenName ?></a></span><span>@<?php echo $messages->username ?></span>
                                    </div>
                                    <div class="msg-box">
                                    ...
                                    <span>
                                    <?php echo $getFromU->timeAgo($messages->messageOn) ?>	
                                    </span>
                                </div>
                            </div>
                            <!--Direct Messages-->
    ...
    {
        $messageForm = $_POST['showChatPopup'];
        $user_id = $_SESSION['user_id'];
        $user = $getFromU->userData($messageForm);
        ?>
        <!-- MESSAGE CHAT START -->
        <div class="popup-message-body-wrap">
        <input id="popup-message-tweet" typ e="checkbox" checked="unchecked"/>
            ...
            <div class="main-msg-footer">
                <div class="main-msg-footer-inner">
                    <ul>
                        ...
                        <li><input id="send" data-user="<?php echo $messageForm ?>" type="submit" value="Send"/></li>
                    </ul>
                </div>
            </div>
